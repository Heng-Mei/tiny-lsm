...
void MemTable::put_(const std::string& key,
...
void MemTable::put(const std::string& key,
  ...
  std::unique_lock<std::shared_mutex> ulock(cur_mtx);
...
auto MemTable::cur_get_(const std::string& key,
...
auto MemTable::frozen_get_(const std::string& key,
                           uint64_t tranc_id) -> SkipListIterator {
  // TODO: Lab2.1 从冻结跳表中查询
  // ? 你需要尤其注意跳表的遍历顺序
  // ? tranc_id 参数可暂时忽略, 直接传递参数即可

  for (auto&& table_ptr : frozen_tables) {
    auto ptr = table_ptr->get(key, tranc_id);
    if (ptr.is_valid()) {
      return ptr;
    }
  }
...
auto MemTable::get_(const std::string& key,
                    uint64_t tranc_id) -> SkipListIterator {
  // TODO: Lab2.1 查询, 无锁版本
...
auto MemTable::get_batch(const std::vector<std::string>& keys,
                         uint64_t tranc_id)
...
void MemTable::remove_batch(const std::vector<std::string>& keys,
  ...
  std::unique_lock<std::shared_mutex> ulock(cur_mtx);
  std::ranges::for_each(
      keys, [&](const std::string& key) { remove_(key, tranc_id); });
}
...
auto MemTable::flush_last(SSTBuilder& builder,
                          std::string& sst_path,
                          size_t sst_id,
...
auto MemTable::iters_monotony_predicate(
    uint64_t tranc_id,
